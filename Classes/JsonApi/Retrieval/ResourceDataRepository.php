...
class ResourceDataRepository implements SingletonInterface
{
    ...
    public function normalize(array $data): array
    {
        ...
        if (isset($data["meta"]) && $data["meta"]["pagination"]) {
            $isCollection = true;
        } else {
            $isCollection = isset($data["data"][0]) && is_array($data["data"][0]) && ! isset($data["attributes"]);
        }
...
//            $baseUri      = $resourceInfo !== null && ! empty($resourceInfo->baseUri) ? $resourceInfo->baseUri : "/resources";
            ...
            if (! empty($m[1]) && $this->configRepository->resource()->isResourceType($m[1])) {
                $resourceType = $m[1];
            }
        }
        ...
        if (empty($link->getHost())) {
            $link = $link->withHost($pageLink->getHost());
        }
    ...
    protected function formatResourceQuery(array $query, string $prefix = ""): string
    {
        ...
        foreach ($query as $k => $v) {
            ...
            if (is_array($v) && ! Arrays::isAssociative($v)) {
                $v = implode(",", $v);
            }
