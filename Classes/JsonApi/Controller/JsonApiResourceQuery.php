	...
	public static function makeNewInstance(ServerRequestInterface $request): JsonApiResourceQuery {
		...
		if (isset($queryParams["sort"]) && !empty($queryParams["sort"])) {
			...
			foreach ($sortList as $field) {
				...
				if ($direction === "desc") $field = substr($field, 1);
		...
		if (isset($queryParams["page"])) {
			if (!is_array($queryParams["page"])) throw new BadRequestException("The page parameter is invalid!");
			if (isset($queryParams["page"]["number"])) $i->page = max(1, (int)$queryParams["page"]["number"]);
			if (isset($queryParams["page"]["size"])) $i->pageSize = max(1, (int)$queryParams["page"]["size"]);
		}
