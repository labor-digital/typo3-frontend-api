...
trait ResourceControllerQueryConstraintsTrait {
	...
	public function addCustomFilterConstraint(BetterQuery $query, ResourceControllerContext $context, string $field, callable $callback): BetterQuery {
		if (!isset($context->getQuery()->getFilters()[$field])) return $query;
	 ...
	 * @throws \League\Route\Http\Exception\BadRequestException
	 */
	protected function addDateRangeFilterConstraint(BetterQuery $query, ResourceControllerContext $context,
													string $startDateProperty, ?string $endDateProperty = NULL, array $options = []): BetterQuery {
		// Prepare options
		...
		else if ($rangeStart > $maxDate) $rangeStart = $maxDate;
		
		// Range end
		$rangeEnd = !empty($filter["rangeEnd"]) ? new DateTimy((string)$filter["rangeEnd"]) : $maxDate;
		if ($rangeEnd < $minDate) $rangeEnd = $minDate;
		else if ($rangeEnd > $maxDate) $rangeEnd = $maxDate;
		...
		if ($options["resolveProperty"] !== "both")
			if ($options["resolveProperty"] === "start") $endDateField = $startDateField;
			else $startDateField = $endDateField;
		...
		foreach ($context->getQuery()->getSorting() as $field => $direction) {
			...
			if (stripos($field, ".") !== FALSE) {
				...
				foreach ($config->includes as $property => $include) {
					if ($include["resourceType"] !== $field) continue;
					$field = $property;
					break;
				}
			}
			...
			// Validate the field
			if (!in_array($field, $allowedFields)) throw new BadRequestException("The sort field " . $field . " is not allowed");
			
			// Add the ordering
			if (!empty($subField)) $field .= "." . $subField;
