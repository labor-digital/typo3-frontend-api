...
class Normalizer implements NormalizerInterface {
	...
	public function normalize($response) {
		...
		// Validate input
		if ($response instanceof stdClass) $response = json_decode(json_encode($response), TRUE);
		...
		foreach ($data as $k => $entity) {
			...
			// Get payload type
			if (empty($payloadType)) $payloadType = $entity["type"];
		...
		if (!is_null($payloadType)) {
			foreach ($result["entities"][$payloadType] as $k => &$entity) {
				if (empty($relationMap[$k])) continue;
				foreach ($relationMap[$k] as $field => $rel) {
					if (!is_array($rel["data"])) continue;
					...
					if ($this->isValidEntity($rel["data"])) {
						...
						foreach ($rel["data"] as $_rel) {
							if (!$this->isValidEntity($_rel["data"])) continue;
		...
		// Build output object
		if (isset($response["meta"])) $result["meta"] = $response["meta"];
		if (isset($response["links"])) $result["links"] = $response["links"];
	...
	protected function addEntity(array $result, array $entity, array &$relationMap = []): array {
		...
		if (empty($entity["relationships"])) return $result;
