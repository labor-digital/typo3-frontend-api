...
abstract class AbstractResourceStrategy extends AbstractStrategy implements StrategyInterface, ContainerAwareInterface {
	...
	protected function convertDbResponse($response) {
		if ($response instanceof BetterQuery) $response = $response->getQuery();
		if ($response instanceof QueryInterface) return $response->execute();
		return $response;
	}
	...
	protected function getContextInstance(string $contextClass, Route $route, ServerRequestInterface $request) {
		...
		$context->setResourceConfig($this->configRepository->resource()->getResourceConfig($context->getResourceType()));
		return $context;
	}
	...
	protected function getManager(ServerRequestInterface $request, ?string $resourceType, $value): Manager {
		...
		if (isset($params["include"])) {
			...
			if (stripos($params["include"], "*") !== FALSE) {
				...
				$transformer = $this->transformerFactory->getTransformer($resourceType)->getConcreteTransformer($value);
	 ...
	 */
	protected function paginateCollection(Collection $collection, ServerRequestInterface $request, ?CollectionControllerContext $context = NULL) {
		$paginator = new Paginator($collection->getData());
		if (!is_null($context)) {
			if (!empty($context->getPageFinder())) $paginator->setPageFinder($context->getPageFinder());
			if (!empty($context->getPageSize())) $paginator->setPageSize($context->getPageSize());
		}
