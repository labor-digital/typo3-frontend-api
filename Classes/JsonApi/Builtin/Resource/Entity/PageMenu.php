...
class PageMenu implements SelfTransformingInterface {
	...
	protected function processSingleMenuItem(string $html, array $additionalFields = []): array {
		...
		if ($dokType === 3) {
			...
			if ($isExternal) $el["type"] = static::TYPE_LINK_EXTERNAL;
			else $el["type"] = static::TYPE_LINK_INTERNAL;
		}
		...
		if (!empty($additionalFields)) {
			...
			foreach ($additionalFields as $field) {
				...
				if (isset($pageInfo[$field])) $el["fields"][$propertyName] = $pageInfo[$field];
				else $el["fields"][$propertyName] = NULL;
			}
		}
