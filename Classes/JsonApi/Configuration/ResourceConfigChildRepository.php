...
class ResourceConfigChildRepository implements FrontendApiConfigChildRepositoryInterface
{
    ...
    public function getResourceTypeByValue($value, bool $returnFallback = true): ?string
    {
        ...
        $type  = $value === null ? null : $this->getResourceTypeByClassName(get_class($value), false);
        if (empty($type)) {
    ...
    {
        return $this->getResourceConfig($resourceType) !== null;
    }
    ...
    public function getSpecialObjectTransformerFor(string $class): ?string
    {
        ...
        foreach (Arrays::attach([$class], class_parents($class), class_implements($class)) as $testClass) {
            if (isset($transformers[$testClass])) {
                return $transformers[$testClass];
            }
        }
