...
class RouteConfig
{
    ...
    {
        return $this->setCacheEnabled($useCache);
    }
    ...
    public function setMiddlewares(array $middlewares, string $middlewareStack = "both"): RouteConfig
    {
        ...
        foreach ($middlewares as $k => $middleware) {
            if (is_numeric($k)) {
                $converted[$middleware] = $def;
            } elseif (! is_array($middleware)) {
                $converted[$k] = $def;
            } else {
                $converted[$k] = array_merge($def, $middleware);
            }
        }
     ...
     * @deprecated will be removed in v10 - will be migrated to constructor injection
     */
