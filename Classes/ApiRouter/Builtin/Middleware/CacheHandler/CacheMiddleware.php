...
class CacheMiddleware implements MiddlewareInterface {
	...
	public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
		...
		if ($searchInServerCache && $this->context->getBeUserAspect()->isLoggedIn() && $this->context->getBeUserAspect()->isAdmin()) {
			if (stripos($request->getHeaderLine("cache-control"), "no-cache") !== FALSE) $searchInServerCache = FALSE;
			else if (stripos($request->getHeaderLine("pragma"), "no-cache") !== FALSE) $searchInServerCache = FALSE;
		}
		...
		if ($searchInServerCache && $this->frontendCache->has($cacheKey)) {
			...
			// Set cache status
			if ($searchInServerCache && $writeServerCache) $cacheStatus = "new";
			else if (!$searchInServerCache && $writeServerCache) $cacheStatus = "force-update";
			else $cacheStatus = "no-cache";
