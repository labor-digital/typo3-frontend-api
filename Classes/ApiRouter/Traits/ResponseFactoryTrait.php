...
trait ResponseFactoryTrait {
	 ...
	 */
	public function injectResponseFactory(ResponseFactoryInterface $factory) {
	...
	public function getResponse(int $code = 200, string $reasonPhrase = ""): ResponseInterface {
		if (!isset($this->responseFactory)) $this->responseFactory = TypoContainer::getInstance()->get(ResponseFactoryInterface::class);
	...
	public function getJsonResponse(array $data, int $code = 200): ResponseInterface {
		...
		return $response->withBody(stream_for(json_encode($data, JSON_PRETTY_PRINT)));
	}
