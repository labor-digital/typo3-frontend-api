...
class ApiMiddlewareFork implements MiddlewareInterface {
	...
	public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
		$router = TypoContainer::getInstance()->get(ApiRouter::class);
		...
		try {
			$staticTemplate = $this->getCurrentSiteConfig()->staticTemplate;
			if (!empty($staticTemplate)) return $this->handleWithStaticTemplate($staticTemplate);
	...
	protected function handleWithStaticTemplate(string $staticTemplate): ResponseInterface {
		...
		if ($templateExists) $content = "<html lang=\"en\"><head><title>Template not found!</title></head><body>The configured, static template was not found!</body></html>";
		else $content = Fs::readFile($staticTemplate);
