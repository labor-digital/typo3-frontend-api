...
class FrontendApiContentElementOption extends AbstractChildExtConfigOption {
	...
	public function registerContentElement(string $contentElementConfigClass, ?string $contentElementName = NULL): FrontendApiContentElementOption {
		if (empty($contentElementName)) $contentElementName = $this->makeContentElementNameFromClassName($contentElementConfigClass);
	 ...
	 */
	public function registerContentElementDirectory(string $directory = "EXT:{{extKey}}/Classes/Controller/ContentElement", bool $asOverrides = FALSE): FrontendApiContentElementOption {
	...
	public function addGlobalElementCssClasses($classes): FrontendApiContentElementOption {
		if (is_string($classes)) $classes = Arrays::makeFromStringList(str_replace(",", " ", $classes), " ");
		if (!is_array($classes)) throw new JsonApiException("Invalid css class list given!");
		$this->config["globalCssClasses"] =
			array_unique(Arrays::attach($this->config["globalCssClasses"], $classes));
		return $this;
	}
	...
	public function __applyTcaOverrides() {
		...
		foreach (get_object_vars($elementConfig) as $k => $v) {
			if (in_array($k, $fieldsToKeep)) continue;
