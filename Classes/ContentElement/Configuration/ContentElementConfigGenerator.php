...
class ContentElementConfigGenerator implements CachedStackGeneratorInterface {
	...
	public function generate(array $stack, ExtConfigContext $context, array $additionalArguments, $option) {
		...
		foreach ($stack as $elementName => $data) {
			$context->runWithFirstCachedValueDataScope($data, function () use (
				$data, $context, $elementName, $tcaTable, $tmp, $additionalArguments
			) {
				
				// Create the configurator
				$configurator = $context->getInstanceOf(ContentElementConfigurator::class, [$elementName, $context, $tcaTable, $additionalArguments["defaultType"]]);
				$tmp->configurators[] = $configurator;
				
				// Run the stack for this element
				$context->runWithCachedValueDataScope($data, function (string $class) use ($context, $elementName, $configurator) {
					
					// Load the config class
					if (!in_array(ContentElementConfigurationInterface::class, class_implements($class)))
					...
					if (empty($configurator->getControllerClass()) && in_array(ContentElementControllerInterface::class, class_implements($class)))
	 ...
	 * @return string
	 */
	protected function makeColumnMap(array $virtualColumns, ContentElementConfigurator $configurator) {
		if (empty($configurator->getModelClass())) return "";
