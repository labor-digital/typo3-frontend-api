...
class ContentElementForm
{
    ...
    public function getTable(): TcaTable
    {
        ...
        if (! $parent instanceof TcaTable) {
            $parent = $parent->getParent();
        }
        if (! $parent instanceof TcaTable) {
            throw new ContentElementConfigException("Could not find the reference to the content element's tca table!");
        }
    ...
    public function __renameVirtualColumns()
    {
        ...
        $conditionProcessor = function ($condition, callable $conditionProcessor) {
            if (is_array($condition)) {
                foreach (Arrays::flatten($condition) as $k => $v) {
                    $condition[$k] = $conditionProcessor($v, $conditionProcessor);
                }

                return Arrays::unflatten($condition);
            }

            if (is_string($condition) && stripos($condition, 'field:') === 0) {
                $cond = explode(":", $condition);
                if (isset($this->virtualColumns[$cond[1]])) {
        ...
        foreach ($this->getFields() as $field) {
            ...
            $field->setDisplayCondition(
                $conditionProcessor($field->getDisplayCondition(), $conditionProcessor)
            );
        }
    }
