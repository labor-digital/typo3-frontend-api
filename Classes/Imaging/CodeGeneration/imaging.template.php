...
function _imagingError(int $code, ?string $msg = NULL, ?string $httpMsg = NULL): void {
	...
	if (empty($httpMsg)) $httpMsg = isset($httpMsgList[$code]) ? $httpMsgList[$code] : "Internal Server Error";
...
call_user_func(function () {
	// Get the image
	if (empty($_GET["file"])) _imagingError(400, "Missing \"file\" parameter!");
	if (!is_string($_GET["file"])) _imagingError(400, "Invalid \"file\" parameter!");
	$file = $_GET["file"];
	...
	if (strlen($hash) !== 32) _imagingError(400, "Invalid hash given!");
	$id = $fileParts[2];
	if (strlen($id) < 2) _imagingError(400, "Invalid file given! The id part is to short!");
	$type = $id[0] === "r" ? "reference" : "file";
	$uid = (int)substr($id, 1);
	if (empty($uid)) _imagingError(404, "Empty or invalid uid given!");
	...
	if ($redirectInfoExists || file_exists($redirectInfoPath)) {
		...
		if ($redirectTarget[0] === "/") $redirectTarget = FRONTEND_API_IMAGING_HOST . $redirectTarget;
